	...
	foreach($data as $ds)
	{
		...
		while($total_page > 0 )
		{
		  ...
		  $page .= $this->printer->table_start();
			...
			{
				$page .= '
				<div style="width:0px; height:0px; position:absolute; left:30%; line-height:0px; top:500px;color:red; text-align:center; z-index:100000; opacity:0.1; transform:rotate(-45deg)">
		  ...
		  if($current_page == $this->printer->total_page)
		  {
		    $qty  = "<b>*** จำนวนรวม  ".number($total_qty)."  หน่วย ***</b>";
				...
				$f_total_bef_vat = number($total_vatable - $order->vat_amount, 2); //number($total_vatable - $total_vat, 2);
